...
const RequestTableRow = ({request}) => {
  console.log(request)
  const {_id, bidderEmail, deadline, jobType, location, price, status, title} = request || {};
  ...
  const handleAccepted = async(status)=>{
    try {
      const result = await changeStatusMutation.mutateAsync({text:status, id:_id});
  ...
  return (
    <tr>
        ...
        <td>${price}</td>
        <td>
            <button onClick={()=>{handleAccepted("Accepted")}} className='bg-[#3b4edb] hover:bg-[#394497] px-3 py-1 font-bold text-white rounded-md me-2'>Accept</button>
            <button onClick={()=>{handleRejected("Rejected")}} className='bg-red-500 hover:bg-red-700 px-3 py-1 font-bold text-white rounded-md'>Reject</button>
        </td>
      </tr>
  )
}
